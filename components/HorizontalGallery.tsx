...
const HorizontalGallery: React.FC<Props> = ({ onSelect }) => {
  ...
  // Adjusted scroll range. 
  // User feedback: "还没到最后就往下了" -> previous -65% was insufficient.
  // Increased to -85% to ensure the "Finis" section is fully revealed.
  const x = useTransform(scrollYProgress, [0, 1], ["0%", "-85%"]);

  return (
    // Increased height to 800vh to provide a longer track for the horizontal scroll, making it feel less rushed.
      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div style={{ x }} className="flex gap-32 px-24 md:px-[15vw] items-center">
          ...
          {ARTWORKS.map((artwork, i) => (
            ...
            >
              {/* 
                  FIX: Enforce fixed width for ALL containers.
                  w-[80vw] md:w-[110vh] ensures a ~16:9 ratio against the 62vh height.
              */}
              <div className="relative h-[55vh] md:h-[62vh] w-[80vw] md:w-[110vh] overflow-hidden bg-zinc-900 shadow-[0_40px_80px_-15px_rgba(0,0,0,0.8)] border border-zinc-800/50">
                <motion.img
                  src={artwork.imageUrl}
                  alt={artwork.title}
                  className="h-full w-full object-cover grayscale group-hover:grayscale-0 transition-all duration-1000 ease-in-out cursor-pointer"
                  whileHover={{ scale: 1.03 }}
                />
                
                <div className="absolute top-4 right-4 mix-blend-difference opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
